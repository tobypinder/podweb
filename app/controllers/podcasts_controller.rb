class PodcastsController < ApplicationController
  ...
  def index
    @sort_by = params[:sort] 
    @sort_by = 'pop' unless ['a-z'].include?(@sort_by)
    if @sort_by == 'pop'
      @podcasts = Podcast.all.includes(:users).sort_by { |podcast| podcast.users.size }.reverse.paginate(page: params[:page], per_page: 12)
    else
  ...
  def show
    ...
    @episodes = @podcast.episodes.sort_by { |episode| episode.publish_date }.reverse.paginate(page: params[:page], per_page: 10)
  end
